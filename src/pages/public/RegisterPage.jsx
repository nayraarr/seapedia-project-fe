...
export default function RegisterPage() {
    ...
    const EyeIcon = () => (
        <button type="button" onClick={() => setShowPassword(p => !p)} className="text-slate-400 hover:text-blue-500 transition">
            {showPassword ? (
                                ...
                                {ROLES.map(role => (
                                    <button
                                        ...
                                        className={`flex-1 py-2.5 rounded-xl border-2 text-sm font-semibold transition ${
                                            form.roles.includes(role.key) ? activeClass[role.color] : inactiveClass
                                        }`}
                                    >
                                        <div>{role.label}</div>
