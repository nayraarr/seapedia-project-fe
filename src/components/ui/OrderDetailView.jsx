                ...
                {order.status === RETURNED ? (
                    <div className="flex items-start justify-between gap-1">
                        {STATUS_FLOW.map((key, idx) => {
                            ...
                            return (
                                <div key={key} className="flex-1 flex flex-col items-center relative">
                                    {idx > 0 && (
                                        <div className={`hidden sm:block absolute -left-[calc(50%+12px)] top-4 w-[calc(100%-24px)] h-0.5 -translate-y-1/2 ${passed ? 'bg-slate-300' : 'bg-slate-200'}`} />
                                    )}
                                    <div className={`relative z-10 w-8 h-8 rounded-full border-2 flex items-center justify-center text-xs font-bold ${passed ? 'bg-slate-300 border-slate-300 text-white' : 'bg-white border-slate-300 text-slate-300'}`}>
                                        {passed ? '✓' : '○'}
                                    </div>
                        ...
                        <div className="flex-1 flex flex-col items-center relative">
                            <div className="hidden sm:block absolute -left-[calc(50%+12px)] top-4 w-[calc(100%-24px)] h-0.5 -translate-y-1/2 bg-red-400" />
                            <div className="relative z-10 w-8 h-8 rounded-full border-2 bg-red-500 border-red-500" />
