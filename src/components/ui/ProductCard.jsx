    ...
    return (
        <div className="card-hover group overflow-hidden">
            <Link to={`/products/${product.id}`} className="block p-4">
                ...
                {/* Image area */}
                <div className={`rounded-xl h-36 mb-4 relative overflow-hidden flex-shrink-0 ${
                    product.imageUrl ? 'bg-slate-100' : `bg-gradient-to-br ${bgGradient}`
                }`}>
                    {product.imageUrl ? (
                        <img
                ...
                <h3 className="font-semibold text-slate-800 text-sm truncate mb-1 group-hover:text-blue-600 transition-colors">
                    {product.name}
                </h3>
